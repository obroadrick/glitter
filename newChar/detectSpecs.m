...
function [imageCentroidsPath, canonicalCentroidsPath] = detectSpecs(P, chardir)
    %% 
    impath = [chardir 'maxImageLeftRight.jpg'];
    % first check if the max image has already been created
    if ~isfile(impath)
        %% get max image
        ims = [];
        idxs = [];
    ...
    % get image points too
    impath = [chardir 'maxImageLeftRight.jpg'];
    C = detectSpecsBetter(impath);
    % get canonical points too
    tform = matfile(P.tform).tform;
