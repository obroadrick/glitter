% Having solved for the surface normals parameterized by the unknown
% characterization-time camera position (and perhaps relative monitor/light
% source position), we now perform a geometric correction to solve for
% those unkown parameters. In particular, we find the parameters most
% consistent with some of the ground-truth parameters known for the system.
% We have done this before with "ground-truth" extrinsic parameters found
% by checkerboards, but we now do this "ground-truth" intrinsic parameters,
% which may be more reliable from the checkerboard estimates. And, a few of
% them, we can assume (if our camera is as high quality as we think it is)
% are certain known values (i.e., zero skew, and optical center as
% expected, which is consistent with matlab tests so far).
...
[error,results] = computeError(xf(1),xf(2),xf(3),MON_WIDTH_MM,...
                              ...
                              train, mostInliersImageSpecPosAll, true);
%}
save([train(1).expdir 'train_after_geometricCorrection'],"results");
...
[error,results] = computeError(xf(1),xf(2),xf(3),MON_WIDTH_MM,...
                              ...
                              test, mostInliersImageSpecPosTest, true);
%}
save([test(1).expdir 'test_after_geometricCorrection'],"results");
...
function [error, results] = computeError(GLIT_TO_MON_PLANES,GLIT_TO_MON_EDGES_X,...
    ...
    for index=1:numcases
        ...
        knownCamPoss{index} = matfile([expdir{index} '/' num2str(index) '/camPosSkew.mat']).camPos; 
        knownCamRots{index} = mat2rod(matfile([expdir{index} '/' num2str(index) '/camRotSkew.mat']).camRot); 
        knownCamIntrinsics{index} = matfile([expdir{index} '/' num2str(index) '/camParamsSkew.mat']).camParams; 
        % also compute the intrinsics
        other.plotStuff = false;
        results(index,:) = [curCamPosEst ...
                    linearEstimateRKglitter(input(index).impath,...
                                            ...
                                            expdir{index}, ...
                                            input(index).skew, other)];
    end
    extrinsicsLoss = false;
    if extrinsicsLoss
        ...
        % Loss is based on intrinsics
        % Normalize each of the intrinsic parameter estimates based on the
        % 'ground-truth' intrinsic parameters, then sum the differences
        % squared.
        
        % First get the matlab/checkerboard results
        %results = [omega1 omega2 omega3 fx fy cx cy s]
        for index=1:numcases
            intr = knownCamIntrinsics{index};
            rot = knownCamRots{index};
            %{
            checkerResults(index,:) = [rot intr.Intrinsics.FocalLength ...
                                        intr.Intrinsics.PrincipalPoint ...
                                        intr.Intrinsics.Skew];
            %}
            checkerResults(index,:) = [intr.Intrinsics.FocalLength ...
                                        intr.Intrinsics.PrincipalPoint];
        end

        % Get the average matlab/checkerboard results
        avgCheckerResults = sum(checkerResults,1) ./ size(checkerResults,1);

        % Normalize the glitter-estimates based on the checkerboard
        % estimates
        whos results
        whos avgCheckerResults
        for index=1:numcases
            normalizedResults(index,:) = results(index,7:10) ./ avgCheckerResults;
        end
        disp(results);
        disp(avgCheckerResults);
        % Sum the squared differences
        error = sum((normalizedResults - 1) .^ 2, "all") / numel(normalizedResults);
    end
end
