...
rng(314159);
inlierThreshold = 10; % a reflected ray is an inlier
...
for counter=1:100
    ...
    if nearestDistLines(specPos(idxs,:), R(idxs,:)) > 2 * inlierThreshold
        continue
    end
    ...
    dists = [];
    for ix=1:size(R,1)
        dists(ix) = distPointToLine(candidate, specPos(ix,:), R(ix,:));
    end
